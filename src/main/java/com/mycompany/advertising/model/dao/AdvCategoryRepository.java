...
public interface AdvCategoryRepository extends JpaRepository<AdvertiseCategoryTo, Long> {
    ...
    @Query(value = "SELECT new com.mycompany.advertising.service.Dto.CategoryIdPair(ac.id , VALUE(cat)) FROM AdvertiseCategoryTo ac LEFT JOIN ac.category cat ON KEY(cat) = ?1 WHERE ac.parent.id = ?2")
    ...
    @Query(value = "SELECT new com.mycompany.advertising.service.Dto.CategoryIdPair(ac.id , VALUE(cat)) FROM AdvertiseCategoryTo ac LEFT JOIN ac.category cat ON KEY(cat) = ?1 WHERE ac.parent IS NULL")
